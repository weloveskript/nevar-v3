...
class Blacklist extends Command {
    ...
    constructor(client) {
        super(client, {
            name: "blacklist",
            description: "administration/blacklist:general:description",
            dirname: __dirname,
            aliases: ["bl"],
    ...
    async run(interaction, message, args, data){
        ...
        let row = new MessageActionRow()
            .addComponents(
                new MessageButton()
                    ...
                    .setLabel(guild.translate("administration/blacklist:main:actions:1"))
                    .setEmoji('➕')
                    .setStyle('SUCCESS'),
                new MessageButton()
                    .setCustomId('blacklist_' + id + '_list')
                    .setLabel(guild.translate("administration/blacklist:main:actions:2"))
                    .setEmoji('📝')
                    .setDisabled(data.guild.plugins.blacklist.list.length === 0)
                    .setStyle('PRIMARY'),
                new MessageButton()
                    .setCustomId('blacklist_' + id + '_remove')
                    .setLabel(guild.translate("administration/blacklist:main:actions:3"))
                    .setEmoji('➖')
                    .setDisabled(data.guild.plugins.blacklist.list.length === 0)
                    .setStyle('DANGER'),
            )
        ...
        if (clicked) {
            if (clicked.customId === 'blacklist_' + id + '_add') {
                let embed = new MessageEmbed()
                    .setAuthor({name: this.client.user.username, iconURL: this.client.user.displayAvatarURL(), url: this.client.website})
                    .setDescription(guild.translate("administration/blacklist:main:collectors:word")
                        .replace('{emotes.arrow}', this.client.emotes.arrow))
                    .setColor(this.client.embedColor)
                    .setFooter({text: data.guild.footer});
                ...
                collectMessage.on("collect", async (msg) => {
                    ...
                    let embed = new MessageEmbed()
                        .setAuthor({name: this.client.user.username, iconURL: this.client.user.displayAvatarURL(), url: this.client.website})
                        .setDescription(guild.translate("administration/blacklist:main:added")
            ...
            if (clicked.customId === 'blacklist_' + id + '_list') {
                let embed = new MessageEmbed()
                    .setAuthor({name: this.client.user.username, iconURL: this.client.user.displayAvatarURL(), url: this.client.website})
                    .setDescription(guild.translate("administration/blacklist:main:list")
                        .replace('{emotes.arrow}', this.client.emotes.arrow)
            ...
            if (clicked.customId === 'blacklist_' + id + '_remove') {
                ...
                collectMessage.on("collect", async (msg) => {
                    ...
                    let embed = new MessageEmbed()
                        .setAuthor({name: this.client.user.username, iconURL: this.client.user.displayAvatarURL(), url: this.client.website})
                        .setDescription(guild.translate("administration/blacklist:main:removed")
                            .replace('{emotes.success}', this.client.emotes.success)
                            .replace('{word}', msg.content))
                        .setColor(this.client.embedColor)
                        .setFooter({text: data.guild.footer});
