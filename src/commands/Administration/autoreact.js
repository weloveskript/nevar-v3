    ...
    async run(interaction, message, args, data){
        ...
        if(args[0].toLowerCase() ===  'add'){
            ...
            if(channel){
                ...
                if(emote.id){
                    for(let val of data.guild.plugins.autoReactChannels){
                        if(val === `${channel.id} | ${emote.id}`){
                            data.guild.plugins.autoReactChannels = data.guild.plugins.autoReactChannels.filter((ch) => ch !== val);
                        }
                    }
                    ...
                    data.guild.plugins.autoReactChannels.push(`${channel.id} | ${emote.id}`);
                    ...
                    if(regex.test(args[2])){
                        ...
                        data.guild.plugins.autoReactChannels.push(`${channel.id} | ${emote}`);
                        await data.guild.save();
                        let embed = new MessageEmbed()
                            .setAuthor(this.client.user.username, this.client.user.displayAvatarURL(), this.client.website)
                            .setDescription(guild.translate("admin/autoreact:added")
        ...
        if(args[0].toLowerCase() === 'remove'){
            ...
            if(channel){
                if(!args[2]){
                    ...
                    let emote = Discord.Util.parseEmoji(args[2]);
                    if(emote.id){
                        if(data.guild.plugins.autoReactChannels.includes(`${channel.id} | ${emote.id}`)){
                            for(let val of data.guild.plugins.autoReactChannels){
                                if(val === `${channel.id} | ${emote.id}`) {
                                    data.guild.plugins.autoReactChannels = data.guild.plugins.autoReactChannels.filter((ch) => ch !== val);
                                    await data.guild.save();
                        ...
                        let regex = /(?:[\u2700-\u27bf]|(?:\ud83c[\udde6-\uddff]){2}|[\ud800-\udbff][\udc00-\udfff]|[\u0023-\u0039]\ufe0f?\u20e3|\u3299|\u3297|\u303d|\u3030|\u24c2|\ud83c[\udd70-\udd71]|\ud83c[\udd7e-\udd7f]|\ud83c\udd8e|\ud83c[\udd91-\udd9a]|\ud83c[\udde6-\uddff]|\ud83c[\ude01-\ude02]|\ud83c\ude1a|\ud83c\ude2f|\ud83c[\ude32-\ude3a]|\ud83c[\ude50-\ude51]|\u203c|\u2049|[\u25aa-\u25ab]|\u25b6|\u25c0|[\u25fb-\u25fe]|\u00a9|\u00ae|\u2122|\u2139|\ud83c\udc04|[\u2600-\u26FF]|\u2b05|\u2b06|\u2b07|\u2b1b|\u2b1c|\u2b50|\u2b55|\u231a|\u231b|\u2328|\u23cf|[\u23e9-\u23f3]|[\u23f8-\u23fa]|\ud83c\udccf|\u2934|\u2935|[\u2190-\u21ff])/g;
                        if(regex.test(args[2])){
                            if(data.guild.plugins.autoReactChannels.includes(`${channel.id} | ${args[2]}`)){
                                for(let val of data.guild.plugins.autoReactChannels){
        ...
        if(args[0].toLowerCase() === 'reset'){
            ...
            if(args[1].toLowerCase() === 'all'){
                data.guild.plugins.autoReactChannels = [];
                await data.guild.save();
                let embed = new MessageEmbed()
                    .setAuthor(this.client.user.username, this.client.user.displayAvatarURL(), this.client.website)
            ...
            if(!channel){
                ...
                for(let val of data.guild.plugins.autoReactChannels){
                    if(val.split(' | ')[0].toString() === channel.id.toString()){
                        data.guild.plugins.autoReactChannels = data.guild.plugins.autoReactChannels.filter((s) => s !== val);
                        bool = true;
                    }
                }
                if(bool){
